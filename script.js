...
function generate(event) {
    ...
    tableAdjacencyMatrix.innerHTML = 
    `<thead>
        <tr>
    ...
    tableIncidenceMatrix.innerHTML = 
    `<thead>
        <tr>
...
// Генерация матрицы смежности неориентированного графа без петель и кратных рёбер
function generateAdjacencyMatrix(n) {
    const matrix = new Array(n).fill(0).map((_, i) => [...Array(n)].map((_, j) => i === j ? 0 : Math.round(Math.random())));
...
function addMultipleEdges(matrix) {
    matrix = matrix.map((_, i) => _.map((v, j) => {
    if (i === j) return 0;
