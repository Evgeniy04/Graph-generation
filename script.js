...
function generate(event) {
	...
	document
		...
		.classList.remove("d-none");
	document
		.querySelector('button[data-bs-target="#showMetricMatrixModal"]')
		.classList.remove("d-none");
...
function createIncidenceMatrix(matrix, edges) {
	const incidenceMatrix = Array.from({ length: matrix.length }, () =>
...
function computeMetricMatrix(R) {
	...
	M = Array.from({ length: R.length }, (_, i) =>
		Array.from({ length: R.length }, (_, j) => (i === j ? 0 : undefined))
	); // Создаём матрицу метрика (такой же размерности, как и R), все элементы которой не определены
